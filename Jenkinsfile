        ...
        stage('Deploy via SSH') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'vm-agent', usernameVariable: 'USER', passwordVariable: 'PASS')]) {
                    bat """
                        "C:\\Program Files\\PuTTY\\plink.exe" -ssh %USER%@192.168.43.178 -pw %PASS% -batch -hostkey "ssh-ed25519 255 SHA256:c0qIOuU0Go/OxAoBbD2H4tdthgZWfSfukR3KxRR0D8E" "docker pull salim178/angular-app:%DOCKER_TAG% && docker run -d -p 80:80 salim178/angular-app:%DOCKER_TAG%"
                    """
                }
            }
        }
